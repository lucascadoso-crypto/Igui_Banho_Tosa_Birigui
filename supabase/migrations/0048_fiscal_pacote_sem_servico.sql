-- Pacotes de fidelidade (catalogo_pacotes) nao tem servico_id vinculado (o
-- modelo so tem nome/frequencia/valor) - por isso a insercao do item fiscal,
-- que so fazia join por sf.servico_id = p.servico_id, nunca encontrava
-- configuracao fiscal para esses pacotes e sempre rejeitava com "Nao foi
-- possivel criar o rascunho fiscal porque o servico do pacote ainda nao
-- possui configuracao fiscal ativa.".
--
-- Como todo servico desta unidade usa o mesmo codigo fiscal (05.08.01 /
-- NBS 1.1405.60.00 - guarda, tratamento, embelezamento e alojamento de
-- animais), quando o pacote nao tem servico_id a funcao passa a usar
-- qualquer servicos_fiscais ativo e configurado da unidade como base (todos
-- resolvem para o mesmo codigo na pratica). Nao muda nada do caminho de
-- agendamento avulso nem do caminho de pacote QUE JA TEM servico_id - so
-- amplia o caso que sempre falhava.
