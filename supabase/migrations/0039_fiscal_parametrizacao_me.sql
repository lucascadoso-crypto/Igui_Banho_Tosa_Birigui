-- Parametrizacao fiscal para NFS-e apos a empresa deixar de ser MEI e virar
-- ME (Simples Nacional), Birigui/SP. Aditiva: so novas colunas/funcoes.
-- Nenhuma credencial de integracao (certificado/senha/token) e adicionada
-- aqui de proposito: Birigui usa o Portal Nacional de NFS-e (ADN), cuja
-- integracao via API exige certificado digital A1 + mTLS. Essa fase e so de
-- estruturacao/parametrizacao; quando a integracao real for implementada, as
-- credenciais devem ficar em Supabase Vault, nunca em coluna de tabela.

begin;

alter table public.config_fiscal_unidade
  add column if not exists cnae_principal text;

alter table public.config_fiscal_unidade
  add column if not exists incentivo_fiscal boolean not null default false;

-- Preparacao para a futura integracao (sem segredo nenhum): o provedor/serie
-- de RPS podem ser definidos desde ja porque nao sao sensiveis.
alter table public.config_fiscal_unidade
  add column if not exists provedor_nfse text not null default 'PORTAL_NACIONAL';

alter table public.config_fiscal_unidade
  add column if not exists serie_rps text;

alter table public.config_fiscal_unidade
  add column if not exists numero_rps_atual bigint;

-- Mesmo algoritmo de digito verificador do CNPJ, seguindo o padrao ja usado
-- em public.cpf_valido (migration 0029).
...
declare
  ...
  i integer;
begin
  v_digits := regexp_replace(coalesce(p_cnpj, ''), '\D', '', 'g');
  ...
  v_sum := 0;
  for i in 1..12 loop
    v_sum := v_sum + (substring(v_digits from i for 1)::integer) * v_pesos1[i];
  end loop;
  v_rest := v_sum % 11;
  v_d1 := case when v_rest < 2 then 0 else 11 - v_rest end;
  if v_d1 <> substring(v_digits from 13 for 1)::integer then
    return false;
  end if;

  v_sum := 0;
  for i in 1..13 loop
    v_sum := v_sum + (substring(v_digits from i for 1)::integer) * v_pesos2[i];
  end loop;
  v_rest := v_sum % 11;
  v_d2 := case when v_rest < 2 then 0 else 11 - v_rest end;
  if v_d2 <> substring(v_digits from 14 for 1)::integer then
    return false;
  end if;
...
-- Ajusta so a descricao fiscal automatica do item de pacote, para refletir
-- "Pacote de banho e tosa - X sessoes" usando o valor do pacote (mesmo
-- comportamento anterior, mesma assinatura da funcao criada em 0019).
